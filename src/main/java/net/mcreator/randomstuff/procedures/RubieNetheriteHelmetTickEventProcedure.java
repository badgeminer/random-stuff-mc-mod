...
public class RubieNetheriteHelmetTickEventProcedure {
	public static void execute(Entity entity) {
		if (entity == null)
			return;
		if (Math.random() >= 0.8) {
			if (ItemTags.getAllTags().getTagOrEmpty(new ResourceLocation("forge:rubienetherite_items")).contains(
					(entity instanceof LivingEntity _entGetArmor ? _entGetArmor.getItemBySlot(EquipmentSlot.FEET) : ItemStack.EMPTY).getItem())
					&& ItemTags.getAllTags().getTagOrEmpty(new ResourceLocation("forge:rubienetherite_items"))
							.contains((entity instanceof LivingEntity _entGetArmor ? _entGetArmor.getItemBySlot(EquipmentSlot.LEGS) : ItemStack.EMPTY)
									.getItem())
					&& ItemTags.getAllTags().getTagOrEmpty(new ResourceLocation("forge:rubienetherite_items")).contains(
